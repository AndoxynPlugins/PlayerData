...
public class ListPlayersFirstJoinCommandHandler implements SubCommandHandler {
    ...
    public void runCommand(CommandSender sender, Command baseCommand, String baseCommandLabel, SubCommand subCommand, String subCommandLabel, String[] subCommandArgs) {
        ...
        for (int i = (pageNumberReal * 6); i < ((pageNumberReal + 1) * 6) && i < pDataList.size(); i++) {
            ...
            messagesToSend.add(ColorList.NAME + current.getUsername() + ColorList.REG + " was first seen " + ColorList.DATA + StaticHelper.getFormattedDate(System.currentTimeMillis() - current.getAllLogins().get(0).getDate()) + ColorList.REG + " ago.");
        }
